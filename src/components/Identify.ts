...
<path d="M5.5 2.5 L5.5 23.5 L10.8 18.6 L14.4 26.5 L18 24.8 L14.3 17.2 L21.5 17.2 Z"
fill="#FFFFFF"
stroke="#0B1620"
stroke-width="1.6"
...
<rect x="22.5" y="7.6" width="2" height="5.2" rx="1"
fill="#FFFFFF"/>
</svg>`;
}
...
export class IdentifyControl implements IControl {
  ...
  private highlightSourceId =
    "__identify_highlight_source";

  private highlightFillId =
    "__identify_highlight_fill";

  private highlightLineId =
    "__identify_highlight_line";

  private highlightPointId =
    "__identify_highlight_point";

  // Fitur yang sedang dipilih melalui klik
  ...
  onAdd(map: MLMap): HTMLElement {
    ...
    const label =
      this.opts.texts?.button ?? "Identify";
    ...
    // Sinkron dengan tool mode global
  ...
  private handleMove = (
    ...
    // Ada objek di bawah cursor
    if (feature) {
      map.getCanvas().style.cursor =
        CURSOR_HIT;

      // Hanya hover kalau belum ada
      // objek yang dipilih lewat klik.
    ...
    // Kalau belum ada objek yang
    // dipilih lewat klik, hilangkan
    // highlight hover.
    if (!this.selectedFeature) {
      this.removeHighlight();
    }
  ...
  private handleClick = (
    ...
    if (features.length === 0) {
      ...
      empty.className =
        "identify-pop identify-pop--empty";

      empty.textContent =
        this.opts.texts?.empty ??
        "Tidak ada fitur di titik ini.";

      popup.setDOMContent(empty);
    } else {
      popup.setDOMContent(
        this.buildContent(features)
      );
    }
  ...
  private query(
    ...
    // Hindari fitur yang sama
    // muncul berkali-kali karena
    // tile terpotong.
    const seen =
      new Set<string>();
  ...
  private featureTitle(
    ...
    const props =
      (f.properties ?? {}) as Record<
        string,
        unknown
      >;

    const candidates =
      this.opts.titleFields ??
      ...
      if (
        ...
        for (const [
          ...
          if (
            ...
            a.className =
              "identify-val identify-val--link";
